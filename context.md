...
**Current phase:** 6 ✅ COMPLETE — Cue Points + Preview fully wired. Audio preview via custom `media://` Electron protocol, singleton HTMLAudioElement in `usePreviewAudio` hook, WaveSurfer.js waveform in `CuePointEditor` modal with hot cues A–H and default cue, SQLite persistence via `cues:update` IPC, library row playing indicator (chartreuse left border + Volume2 icon), timeline card Play/Pause toggle. Ready for Phase 7 (Export + Validation) on go-ahead.
