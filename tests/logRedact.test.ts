...
import {
  ...
  redactPath,
  scrubString
} from '../electron/services/logging/redact'
...
describe('redactPath', () => {
  it('collapses the home dir to ~', () => {
    expect(redactPath(`${HOME}/Documents/SetRecord/app.db`)).toBe('~/Documents/SetRecord/app.db')
  })
...
describe('isSensitiveLogKey', () => {
  it('flags user-content and secret keys across casing styles', () => {
    for (const k of [
      'email',
      'userEmail',
      'customer_email',
  ...
  it('leaves benign structural/diagnostic keys alone', () => {
    for (const k of ['count', 'durationMs', 'status', 'sid', 'version', 'ok', 'index', 'bpm']) {
