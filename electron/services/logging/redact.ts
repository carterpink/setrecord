...
/** Placeholder written in place of a value under a sensitive log key. */
export const REDACTED = '[redacted]'

// Key words that mark a ctx field as user-content or a secret. A field whose key
// contains any of these (camelCase/underscore/dot segments) has its value
// dropped wholesale, regardless of the value's shape — the path/email scrubbers
// only catch strings of a known shape, so a track title or a raw key passed as a
// plain string (or number/object) under one of these keys would otherwise slip
// through. Over-redaction in logs is a deliberate, privacy-positive trade-off.
...
/** Split a key into lowercased word segments (camelCase + delimiters). */
...
/** True when a log-context key names a sensitive field (see {@link SENSITIVE_WORDS}). */
export function isSensitiveLogKey(key: string): boolean {
  return keyWords(key).some((w) => SENSITIVE_WORDS.has(w))
}
