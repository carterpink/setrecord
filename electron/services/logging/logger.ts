...
import { frameBasename, isSensitiveLogKey, redactPath, REDACTED, scrubString } from './redact'
...
function redactCtx(ctx: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const [k, v] of Object.entries(ctx)) {
    if (isSensitiveLogKey(k)) {
      // Sensitive field (track title, venue, path, key, email, …): drop the
      // value wholesale regardless of type — don't rely on shape-based scrubbing.
      out[k] = REDACTED
